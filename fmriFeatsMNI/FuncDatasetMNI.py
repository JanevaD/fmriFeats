...
import LoopFmriprepOutputMNI as LoopFmriprepOutput
...
P_N = [[100,7]]
functional_datasets= LoopFmriprepOutput.loop_fmriprep_output(root,  preprocessing, final_confounds)
...
for dataset in  functional_datasets:
   ...
   # preproc = dataset["preprocessing"]
    ...
    for i, key in enumerate(functional_datasets["data"].keys()):
        ...
        seg = pd.concat([seg,functional_datasets['data'][key]['seg'].T],axis = 1)
        integ = pd.concat([integ,functional_datasets['data'][key]['integ']],axis = 1)
        networks = functional_datasets['data'][key]['seg'].index
...
for dataset in  functional_datasets:
    ...
    for i, key in enumerate( functional_datasets["data"].keys()):
        ...
        seg = pd.concat([seg,pd.DataFrame( functional_datasets['data'][key]['falff'])],axis =1)
        integ = pd.concat([integ,pd.DataFrame( functional_datasets['data'][key]['alff'])],axis = 1)
...
for dataset in  functional_datasets:
    ...
    for i, key in enumerate( functional_datasets["data"].keys()):
        ...
        seg = pd.concat([seg, functional_datasets['data'][key]['dfcs_mean_segs'].T],axis =1)
        integ = pd.concat([integ, functional_datasets['data'][key]['dfcs_mean_integs']],axis = 1)
        networks =  functional_datasets['data'][key]['fc'].index
...
#%%
data = functional_datasets.get('data')
fcs=[]
